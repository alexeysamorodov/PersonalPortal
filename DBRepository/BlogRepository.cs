...
namespace DBRepository
{
    ...
    {
        public BlogRepository(string connectionString, IRepositoryContextFactory contextFactory) : base(
        ...
        public async Task<Page<Post>> GetPosts(int index, int pageSize, string tag = null)
        {
            ...
            using (var context = ContextFactory.CreateDbContext(ConnectionString))
            {
                ...
                result.Records = await query.Include(p => p.Tags).Include(p => p.Comments)
                    .OrderByDescending(p => p.CreatedDate).Skip(index * pageSize).Take(pageSize).ToListAsync();
            }
        ...
        public async Task<Post> GetPost(int postId)
        {
            using (var context = ContextFactory.CreateDbContext(ConnectionString))
            {
                return await context.Posts.Include(p => p.Tags).Include(p => p.Comments)
